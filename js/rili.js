...
//	return new Date(fird);
	//循环得到要显示的所有日期数据
	/**返回数据格式
	 * (("日期Ly-m-d","年","月","日","农历","星期","是否今天","班次标识 1,2,3","是否当月"),(.......)) 
	 */
	for (var i = 0; i< 42;i++) {
		...
		reDate[i][7] = huanban(mdate.getFullYear(),mdate.getMonth(),mdate.getDate());
...
//		8 返回当前日期是否和查询月相同
	...
	return reDate;
//	alert(calendar.solar2lunar(2015,11,16).IDayCn);
}
/**
 * 换班色彩渲染
 * @param  y,m,d 需要渲染的日期  年,月,日
 * @return 1,2,3
 */
function huanban(y,m,d){
	...
	*/
	...
	if (nian != null){
		...
		var banci = new Date(2016,1,11);//开始上班的第一天
	}
//	var banci = new Date(first_y,first_m,first_d);//开始上班的第一天
	var chadate = new Date(y,m,d);
	var result = 0;
	if(chadate.getTime() > banci.getTime()){
		//获得两个日期之间的间隔
		result = (chadate.getTime() - banci.getTime()) / 1000 / 24 / 60 / 60 ;
		
		result = result > 11 ? result % 12 : result;
		if(result < 4){
			result = 1;
		}else if(result > 7 ){
			result = 3;
	...
	}else if(chadate.getTime() == banci.getTime()){ 
		result = 1;	
	}else{
		result = (banci.getTime() - chadate.getTime())  / 1000 / 24 / 60 / 60;
...
function mycalendar(y,m,d){
	var htm = document.getElementById('day');
	htm.innerHTML = "";
//	获取当前显示的数据
	var b = firstDay(y,m);
	for (var i = 0;i<42;i++){
		var newhtml = document.createElement('li');
		newhtml.setAttribute("class","day-tap day-"+b[i][8]);
		var str ='';
		if(b[i][7] == 1){
			str = "<em  class='work color-red'>白</em>";
		}else if(b[i][7] == 2){
			str = "<em  class='work color-blue'>夜</em>";
		}else{
			str = "<em  class='work color-green'>休</em>";
		}
		if(b[i][6] == 1 ){
			newhtml.innerHTML = str +"<div class='rili-box select-day today'><div class='rili-tian' nian='"+b[i][1]+"'yue='"+b[i][2]+"'>"+b[i][3]+"</div><div class='rili-nongli hidd'>"+b[i][4]+"</div></div>";
		}else{
			newhtml.innerHTML =str +"<div class='rili-box'><div class='rili-tian' nian='"+b[i][1]+"'yue='"+b[i][2]+"'>"+b[i][3]+"</div><div class='rili-nongli hidd'>"+b[i][4]+"</div></div>";
		}
		htm.appendChild(newhtml);
	}
//	newhtml.innerHTML = "<div class='zhanwei'></div>";
//	htm.appendChild(newhtml);
}

//<li>
//				<em  class="work">白</em>
//			<div class="rili-box select-day">
//				<div class="rili-tian ">
//					1
//				</div>
//				<div class="rili-nongli hidd">
//					初一
//				</div>
//			</div>
//		</li>